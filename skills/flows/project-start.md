...
> **Ativado por:** após **"sim"** no `flows/kickoff.md` (comando *Começar projeto*)  
> **Antes deste arquivo:** kickoff — perguntas + confirmação  
> **Governança de capability:** `flows/skills-governance.md`
...
- `skills/core/runtime/*.yaml` criado como fonte operacional da IA
...
```
[1] Copiar templates/runtime/ → skills/core/runtime/ (inclui schema/ + index.yaml + validate.py)
        ↓
...
Consultar `Start.md` + `flows/skills-governance.md`:

1. Verificar `catalog/project-starter.skill`
...
├── skills/core/runtime/*.yaml   → operacional IA ← carregado toda sessão
├── [código — structure skill]
└── skills/outputs/         → ROADMAP, BRIEF, ARCHITECTURE (humano)
```

Templates:
- `templates/runtime/`       → skills/core/runtime/*.yaml
...
- [ ] `skills/core/runtime/` + `schema/` criados de `templates/runtime/`
- [ ] `python3 skills/core/runtime/validate.py` → 0 failed
