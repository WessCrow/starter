...
def fake_repo(tmp_path: Path) -> Path:
    ...
    # Skills skeleton (Clean Architecture layers)
    skills = tmp_path / "skills"
    (skills / "flows").mkdir(parents=True)
    (skills / "catalog").mkdir(parents=True)
    ...
    (skills / "infra" / "scripts").mkdir(parents=True)
    (skills / "core" / "runtime").mkdir(parents=True)

    # Bootstrap script mínimo
    bootstrap = skills / "infra" / "scripts" / "clean-framework-artifacts.sh"
