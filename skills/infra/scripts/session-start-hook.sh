...
#     "SessionStart": [
#       { "hooks": [ { "type": "command",
#         "command": "bash skills/infra/scripts/session-start-hook.sh" } ] }
#     ]
#   }
# }
...
1. Carregar skills/core/runtime/index.yaml (hot: rules.yaml, context.yaml, state.yaml).
2. Seguir skills/flows/Start-ops.md como orchestrator da sessao.
3. 'Comecar projeto' -> skills/flows/kickoff.md. Nova feature -> skills/flows/feature-flow.md (HARD-GATE: sem codigo antes de contrato aprovado).
4. Bug/erro -> skills/flows/debugging-protocol.md. Feedback/critica -> skills/flows/review-reception.md.
5. Nunca afirmar 'pronto/corrigido' sem evidencia: skills/catalog/verify-before-done.skill.
...
if [ ! -f "${REPO_ROOT}/skills/core/runtime/index.yaml" ]; then
  CONTEXT="AVISO: skills/core/runtime/index.yaml nao encontrado. Estrutura STARTER incompleta — verificar se a pasta skills/ foi copiada."
fi
...
if [ -d "$INTAKE_DIR" ]; then
  ...
  if [ "$pending_count" -gt 0 ]; then
    CONTEXT="${CONTEXT}
LEMBRETE: ${pending_count} skill(s) candidata(s) TESTAR_ aguardando avaliacao em docs/private/_novas skills/ — protocolo: skills/flows/skill-intake.md."
  fi
fi
