...
# Uso: bash skills/infra/scripts/upgrade-from-starter.sh [caminho-do-projeto-filho]
# Padrão: diretório atual (.)
#
# Copia APENAS:
#   - skills/templates/runtime/validate.py → skills/core/runtime/validate.py
#   - skills/templates/runtime/schema/     → skills/core/runtime/schema/
...
if [[ ! -d "$DST_RUNTIME" ]]; then
  echo "ERRO: $DST_RUNTIME não existe — projeto filho precisa ter skills/core/runtime/"
  exit 1
fi

FRAMEWORK_V="$(grep -E '^framework_v:' "$STARTER_ROOT/skills/core/runtime/state.yaml" 2>/dev/null | awk '{print $2}' | tr -d '"' || echo "unknown")"
