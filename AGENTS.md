...
## 0i. Como implementar — escada YAGNI (antes de escrever código)

Antes de escrever qualquer código, percorra os degraus em ordem e **pare no primeiro que se sustenta**:
...
**Exceção de governança (STARTER):** o degrau 1 ("isso precisa existir?") **não se aplica** a arquivos de runtime (`skills/core/runtime/*.yaml`), validators (`validate.py`), hooks de segurança (`host-guard.sh`) nem aos gates de QA — esses existem por governança, não por demanda de feature, e nunca são "omitidos" por YAGNI.
